...
definePageMeta({ layout: 'dynamic', pageTitle: 'Mes leads' })
...
const decennalStatus = computed(() => profile.value?.decennal_status ?? 'none')
const canUnlockLeads = computed(() => decennalStatus.value === 'valid')
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 reveal">
      ...
      >
        ...
        <template v-if="lead.status === 'locked'">
          ...
          <div class="px-5 py-3">
            <NuxtLink
              v-if="freeRemaining > 0 && canUnlockLeads"
              :to="`/espace/leads/${lead.id}`"
              class="inline-flex items-center justify-center gap-2 w-full h-9 px-4 rounded-full bg-safety text-white text-xs font-semibold hover:scale-105 shadow-safety/20 transition-transform"
            >
              Débloquer (1 crédit gratuit)
            </NuxtLink>
            <span
              v-else-if="freeRemaining > 0 && !canUnlockLeads"
              class="inline-flex items-center justify-center gap-2 w-full h-9 px-4 rounded-full bg-slate-200 text-slate-400 text-xs font-semibold cursor-not-allowed"
              title="Envoyez vos documents (KBIS + Décennale) pour débloquer l'accès aux leads"
            >
              Débloquer (1 crédit gratuit)
            </span>
            <NuxtLink
              ...
              :class="canUnlockLeads ? 'inline-flex items-center justify-center gap-2 w-full h-9 px-4 rounded-full bg-safety text-white text-xs font-semibold hover:scale-105 shadow-safety/20 transition-transform' : 'inline-flex items-center justify-center gap-2 w-full h-9 px-4 rounded-full bg-slate-200 text-slate-400 text-xs font-semibold cursor-not-allowed'"
            >
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/>
              </svg>
