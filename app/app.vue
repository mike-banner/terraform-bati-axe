  ...
  <div class="min-h-screen bg-background text-foreground flex flex-col font-sans antialiased">
    ...
    <header class="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur-sm">
      <div class="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <NuxtLink
          to="/"
          class="text-base font-bold tracking-tight text-foreground hover:opacity-70 transition-opacity"
        >
          BÂTI-AXE
        </NuxtLink>
    ...
    <footer class="border-t border-border mt-auto">
      <div class="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-muted-foreground">
        <p>&copy; {{ new Date().getFullYear() }} BÂTI-AXE. Tous droits réservés. Conforme RGPD.</p>
        <nav class="flex items-center gap-5">
          <NuxtLink to="/legal/mentions-legales" class="hover:text-foreground transition-colors">
            Mentions légales
          </NuxtLink>
          <NuxtLink to="/legal/confidentialite" class="hover:text-foreground transition-colors">
            Confidentialité
          </NuxtLink>
          <NuxtLink to="/legal/cgu" class="hover:text-foreground transition-colors">
            CGU
          </NuxtLink>
        </nav>
      </div>
