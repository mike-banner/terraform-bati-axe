...
|---|---|---|---|---|
| P1 | **Analytics de conversion** (simulateur → lead → contact → chantier signé). Reco : **Matomo** (open-source, GDPR, self-hostable — fit marché FR) ou **PostHog** (funnels + session replay clés-en-main). Axiom ≠ analytics (c'est de la gestion de logs) — écarté pour P1. Décision à trancher avant implémentation | Avis mentor | Sans mesure, impossible de savoir si le pilote fonctionne — plus critique que l'admin | ❌ à trancher |
