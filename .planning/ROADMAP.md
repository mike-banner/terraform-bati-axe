...
- [x] **Phase 05.13: Dette technique + P9 Mobile QA + P5 Feedback Loop** - Suite e2e Playwright câblée sur le Chrome système (channel 'chrome', bundle Chromium cassé en sandbox) + specs simulateur réécrites pour le flux 6 étapes + test badge réparé (bg-[#F8FAFC]) + branche test-fix supprimée ; QA mobile mesurée (scripts audit overflow/tactile), cibles tactiles ≥ 44px (header/simulateur/tunnel/footer), état vide catégorie leads atteignable, projet Playwright `mobile-chromium` (48/48 specs desktop+mobile) ; feedback loop « refus → marché » extrait dans `handleLeadDecision.ts` + 9 tests unitaires (remise au marché, garde-fou MAX_RELAUNCHES, 403/404). **Phase complète 3/3** (livré 2026-08-23)
- [ ] **Phase 05.14: FIX Storage R2 Multi-Buckets & Terraform Prod** - Fix de la lecture des bindings `NUXT_R2_*` dans les 7 endpoints API server + isolation 3 buckets (`batiaxe-public-[env]`, `batiaxe-vault-[env]`, `batiaxe-b2b-[env]`) + propagation Terraform et GitHub Actions Prod.
- [x] **Phase 05.15: FIX Verrouillage Leads non-vérifiés** - Blocage strict du déblocage des leads : gate serveur `decennal_status !== 'valid'` sur `leads/[id]/claim.patch.ts` (l'UI seule ne suffisait pas, contournable par appel API direct) + alerte email admin quand le SIRET n'est pas confirmé actif au claim. **Phase complète 1/1** (livré 2026-08-29)
...
**Success Criteria** (what must be TRUE):
  ...
  2. Un push sur `dev` déclenche le workflow Terraform Dev et met à jour Cloudflare Pages Dev (`dev.bati-axe.fr`).
...
### Phase 05.13: Dette technique + P9 Mobile QA + P5 Feedback Loop (INSERTED 2026-08-23)

**Goal:** Soldée en une phase les trois chantiers demandés par l'utilisateur (« on fait les dettes techniques pour commencer + P9 + P5 ») : remettre la suite e2e Playwright en état de marche, auditer et corriger le mobile (P9) avec des états vides vérifiés, et tester le feedback loop « refus → remise au marché » (P5) qui n'avait jamais été vérifié de bout en bout.

**Contexte:** la suite e2e n'avait jamais été lancée (bundle Chromium cassé en sandbox) et ses specs simulateur dataient de l'ancien flux ; `tests/badges.test.ts` échouait (dérive de palette) ; la branche locale `test-fix` était du déchet docs. P9 (mobile) n'avait jamais été audité en viewport réel. P5 : la logique `shouldRelaunch` était unit-testée et câblée dans `decision.post.ts`, mais l'orchestration complète (décision → tous refusés → leads 'lost' + relaunch_count + email) n'était pas testée.

**Depends on:** Phase 5.6/05.9 (simulateur 6 étapes), Phase 6/06-03 (feedback loop REQ-06), Phase 05.12 (header/footer boutons pill). Aucune migration DB ni endpoint nouveau.

**Success Criteria** (what must be TRUE):
  1. Suite e2e Playwright lancée et verte sur le Chrome système (channel 'chrome') : 24 specs passent (avant : jamais lancée).
  2. `tests/badges.test.ts` vert (composant aligné sur la palette Sketch 001, `bg-[#F8FAFC]`).
  3. Aucun débordement horizontal à 320/390px sur /, /simulateur, /partenaires, /b2b/partenaires, /mon-projet ; cibles tactiles principales ≥ 44px ; état vide de catégorie des leads atteignable ; projet Playwright `mobile-chromium` qui rejoue les specs en mobile (48/48).
  4. Feedback loop testé : orchestration extraite dans `server/utils/handleLeadDecision.ts` + 9 tests unitaires couvrant remise au marché, non-relance (pending/selected/aucun engagé/MAX_RELAUNCHES), 403/404.
  5. Outils d'audit réutilisables dans `scripts/` (mobile-audit, touch-audit, monprojet-mobile).
...
| P9 | **Mobile QA landing + simulateur + états vides** (dashboard sans leads, espace client sans messages, erreurs/offline) | Spec 06/08 + avis mentor | 80% de la première impression d'un nouveau user | ✅ fait (2026-08-23, phase 05.13-02 : audit + cibles ≥ 44px + projet e2e mobile 48/48) |
| P12 | **Page pro public « digne de ce nom »** — base immersive, galerie, badges et CTA contact livrés; avis clients reportés à la Phase 7 | Décision utilisateur 2026-08-19 | C'est ce que voient les particuliers avant de contacter un pro | ✅ base livrée — avis en Phase 7 |
...
- **Vérification automatique OCR & AI des Décennales / KBIS (Mindee / Vision API)** — extraction automatique des dates d'expiration, numéro de police et compagnies d'assurance depuis les PDF/Images R2 avec système de fallback validation 1-click admin (v2).
