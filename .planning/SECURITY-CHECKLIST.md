...
- ❌ **2 restent volontairement non corrigées** : `nuxt` 4.0.0-4.5.0 (high — inclut une RCE via injection de template dans les Server Island Props, une fuite de payload SSR cross-user, et un contournement d'auth sur routes casse-mixte) + `@nuxt/vite-builder` qui en dépend. Le fix existe (`nuxt@4.5.2`) et passe l'audit (0 vulnérabilité), **mais casse le build de prod** : `TypeError: trustedFunctions.difference is not a function` (nuxt 4.5.2 embarque un nouveau peer `rolldown@~1.2.1`, incompatible avec l'environnement actuel). Testé, confirmé cassé, **reverté proprement** (`git checkout -- package.json package-lock.json` + reinstall, build revérifié OK après retour arrière).
**Action** : la vraie vulnérabilité RCE Nuxt reste ouverte. Nécessite d'investiguer la compat rolldown/Node séparément (probablement un upgrade Node vers 22+ requis — plusieurs autres paquets du projet réclament déjà `>=22` en warning EBADENGINE) avant de retenter le bump nuxt. À traiter en priorité mais pas à la légère vu que ça a cassé le build au premier essai.
